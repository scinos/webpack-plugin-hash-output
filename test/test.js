...
const expectAssetNameToContainContentHash = (stats) => {
    ...
    Object.keys(assets).filter(file => !isSecondaryFile(file)).forEach((name) => {
...
describe('OutputHash', () => {
    ...
    modes.forEach((mode) => {
        context(`In ${mode} mode`, () => {
            ...
            it('Works with common chunks', () => webpackCompile('common-chunk', mode)
                .then(sanityCheck));

            it('Works with async chunks', () => webpackCompile('async-chunk', mode)
                .then(sanityCheck)
                .then((stats) => {
            ...
            it('Works with runtime chunks', () => webpackCompile('runtime-chunk', mode)
                .then(sanityCheck)
                .then((stats) => {
                    const asyncAsset = asset(stats, '0');
            ...
            it('Works when runtime has common chunks that require async files', () => webpackCompile('common-runtime-chunk', mode)
                .then(sanityCheck)
                .then((stats) => {
